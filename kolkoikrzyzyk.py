...
# tytuł okna gry
pygame.display.set_caption('Kółko i krzyżyk - zaliczenie ML')
...
# narysowanie planszy, czyli linii oddzielającej pola
...
# postaw przeciwny znak (coś ten krzyżyk mi nie wychodził, więc kółko ale w innym kolorze)
...
def sprawdz_pola(uklad, wygrany=None):
    ...
    for lista in POLA_INDEKSY:
        ...
        for ind in lista:
            kol.append(POLE_GRY[ind])  # zapisz wartość z POLE_GRY
        if (kol in uklad):  # jeśli znalazłeś układ wygrywający lub blokujący
            # zwróć wygranego (1,2) lub indeks pola do zaznaczenia
            wartosc = wygrany if wygrany else lista[kol.index(0)]

    return wartosc

# ruchy komputera jak i główne zagadnienie AI w projekcie


def ai_ruch(RUCH):
    pole = None  # które pole powinien zaznaczyć komputer

    # listy wielowymiarowe, których elementami są listy zagnieżdżone
    ...
    # jeżeli nie można wygrać + gracza nie trzeba blokować, wylosuj pole
    while pole is None:
...
# sprawdź kto wygrał lub czy jest remis


def kto_wygral():
    # układy wygrywające dla obu zawodników
    uklad_gracz = [[1, 1, 1]]
    uklad_komp = [[2, 2, 2]]
...
# funkcja wyświetla komunikat końcowy tworzy nowy obraz + tekst, pobiera obszar i rysuje w grze
...
while True:
        ...
        if WYGRANA is False:
            if RUCH == 1:
                if event.type == MOUSEBUTTONDOWN:
                    if event.button == 1:  # jeśli naciśnięto 1 przycisk
    ...
    OKNOGRY.fill((0, 0, 0))  # kolor powierzchni RGB
    rysuj_plansze()
    rysuj_pole_gry()
    if WYGRANA:
